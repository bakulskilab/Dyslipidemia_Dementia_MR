...
model <- lapply(var_names, function(x) {
  lm(substitute(PGS_HDL ~ i, list(i = as.name(x))), data = afric_new)
})
...
pdf("~/Desktop/HDL_Forest(color)_Afric_0717.pdf",width = 10, height = 6)
forestplot(labeltext = tabletext, graph.pos = 1,
           ...
           Wave 2006 to 2012, African ancestry sample (n = 2134)",
...
model <- lapply(var_names, function(x) {
  lm(substitute(PGS_TC ~ i, list(i = as.name(x))), data = afric_new)
})
...
pdf("~/Desktop/TC_Forest(color)_African_0717.pdf",width = 10, height = 6)
forestplot(labeltext = tabletext, graph.pos = 1,
           mean = c(NA, NA, forest.data$beta), 
           lower = c(NA, NA, forest.data$lower_CI),
           upper = c(NA, NA, forest.data$upper_CI),
